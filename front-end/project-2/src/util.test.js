...
test('Item Name not allowed to be Empty', () => {
    ...
    expect(encodeItem(null, 3, "", 0.05, 2)).toBeNull();
});
...
test('Price value cannot be NaN', () => {
    window.alert.mockClear();
    expect(encodeItem(null, 3, "test123", "abc", 2)).toBeNull();
});
test('Price value cannot be negative', () => {
    window.alert.mockClear();
    expect(encodeItem(null, 3, "test123", -1.23, 2)).toBeNull();
});
test('Price value cannot be zero', () => {
    window.alert.mockClear();
    expect(encodeItem(null, 3, "test123", 0, 2)).toBeNull();
});
test('Quantity value cannot be NaN', () => {
    window.alert.mockClear();
    expect(encodeItem(null, 3, "test123", 0.05, "xyz")).toBeNull();
});
test('Quantity value cannot be negative', () => {
    window.alert.mockClear();
    expect(encodeItem(null, 3, "test123", 0.05, -2)).toBeNull();
});
test('Quantity value cannot be zero', () => {
    window.alert.mockClear();
    expect(encodeItem(null, 3, "test123", 0.05, 0)).toBeNull();
});
test('Quantity value cannot be a float', () => {
    window.alert.mockClear();
    expect(encodeItem(null, 3, "test123", 0.05, 0.02)).toBeNull();
});
test('Successful bill item submit (integer price)', () => {
    window.alert.mockClear();
    expect(encodeItem(null, 3, "test123", 7, 2)).not.toBeNull();
});
test('Successful bill item submit (float price)', () => {
    window.alert.mockClear();
    expect(encodeItem(null, 3, "test123", 0.05, 2)).not.toBeNull();
});
/*
test('temp', () => {
    window.alert.mockClear();
